...
public class OceaniaW3 extends World
{
    ...
    public OceaniaW3() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new Oceania3(),665,219);//añade la imagen al mundo
        addObject (new OW4(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(oceania3A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(oceania1A) || Greenfoot.mouseClicked(oceania2A) || Greenfoot.mouseClicked(oceania4A) || Greenfoot.mouseClicked(oceania5A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        { 
            ...
            addObject (oceania5A,1091,535);
        }
        else if( order == 1)//segundo orden aleatorio
        { 
            addObject (oceania4A,1091,535);
            addObject (oceania3A,239,533);
            addObject (oceania2A,668,532);
        }
        else if( order == 2)//tercer orden aleatorio
        {
            addObject (oceania2A,668,532);
            addObject (oceania3A,1091,535);
            addObject (oceania5A,239,533);
        }
    }

    public void addScore(int points)//añade el puntaje al mundo
    {
        score = OceaniaW2.score + points; //suma los puntos correctos
