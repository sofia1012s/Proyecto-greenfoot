...
public class EuropaW3 extends World
{
    ...
    public EuropaW3() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new Europa3(),665,219);//añade la imagen al mundo
        addObject (new EW4(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(europa3A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(europa1A) || Greenfoot.mouseClicked(europa2A) || Greenfoot.mouseClicked(europa4A) || Greenfoot.mouseClicked(europa5A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        { 
            addObject (europa3A,239,533);
            addObject (europa1A,668,532);
            addObject (europa5A,1091,535);
        }
        else if( order == 1)//segundo orden aleatorio
        { 
            addObject (europa2A,1091,535);
            addObject (europa3A,239,533);
            addObject (europa1A,668,532);
        }
        else if( order == 2)//tercer orden aleatorio
        {
            addObject (europa4A,668,532);
            addObject (europa2A,1091,535);
            addObject (europa3A,239,533);
        }
    }
    ...
    {
        score = EuropaW2.score + points; //suma los puntos correctos
