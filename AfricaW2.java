...
public class AfricaW2 extends World
{
    ...
    public AfricaW2() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new Africa2(),665,219);//añade la imagen al mundo
        addObject (new AW3(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(africa2A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(africa1A) || Greenfoot.mouseClicked(africa3A) || Greenfoot.mouseClicked(africa4A) || Greenfoot.mouseClicked(africa5A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        { 
            addObject (africa1A,239,533);
            addObject (africa2A,668,532);
            addObject (africa4A,1091,535);
        }
        else if( order == 1)//segundo orden aleatorio
        { 
            addObject (africa2A,1091,535);
            addObject (africa3A,239,533);
            addObject (africa1A,668,532);
        }
        else if( order == 2)//tercer orden aleatorio
        {
            addObject (africa4A,668,532);
            addObject (africa1A,1091,535);
            addObject (africa2A,239,533);
        }
    }

    public void addScore(int points)//añade el puntaje al mundo
    {
        score = AfricaW1.score + points; //suma los puntos correctos
