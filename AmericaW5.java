...
public class AmericaW5 extends World
{
    ...
    public AmericaW5() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new America5(),665,219);//añade la imagen al mundo
        addObject (new MWS(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(america5A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(america1A) || Greenfoot.mouseClicked(america2A) || Greenfoot.mouseClicked(america3A) || Greenfoot.mouseClicked(america4A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        { 
            ...
            addObject (america5A,668,532);
            addObject (america3A,1091,535);
        }
        else if( order == 1)//segundo orden aleatorio
        { 
            addObject (america2A,1091,535);
            addObject (america1A,239,533);
            addObject (america5A,668,532);
        }
        else if( order == 2)//tercer orden aleatorio
        {
            addObject (america4A,668,532);
            addObject (america5A,1091,535);
    ...
    {
        score = AmericaW4.score + points; //suma los puntos correctos
