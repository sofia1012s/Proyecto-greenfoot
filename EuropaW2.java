...
public class EuropaW2 extends World
{
    ...
    public EuropaW2() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new Europa2(),665,219);//añade la imagen al mundo
        addObject (new EW3(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(europa2A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(europa1A) || Greenfoot.mouseClicked(europa3A) || Greenfoot.mouseClicked(europa4A) || Greenfoot.mouseClicked(europa5A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        else if( order == 2)//tercer orden aleatorio
        {
            ...
            addObject (europa2A,239,533);
        }
    }

    public void addScore(int points)//añade el puntaje al mundo
    {
        score = EuropaW1.score + points; //suma los puntos correctos
