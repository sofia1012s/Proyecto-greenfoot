...
public class AsiaW5 extends World
{
    ...
    public AsiaW5() //agregando objetos al mundo
    {    
        super(1300, 700, 1); //tamaño del mundo
        addObject (new Asia5(),665,219);//añade la imagen al mundo
        addObject (new WSS(),1249,326);//añade boton siguiente para pasar a la siguiente pregunta
    ...
    public void act()//añade 1 punto al contador
    {
        if (clickCount<1)
        {
            if (Greenfoot.mouseClicked(asia5A))//Respuesta correcta
            {
            ...
            if (Greenfoot.mouseClicked(asia1A) || Greenfoot.mouseClicked(asia2A) || Greenfoot.mouseClicked(asia3A) || Greenfoot.mouseClicked(asia4A))
    ...
    public void Order() //permite tener las opciones de respuestas en ordenes distintos 
    {
        ...
        else if( order == 2)//tercer orden aleatorio
        {
            ...
            addObject (asia5A,1091,535);
            addObject (asia2A,239,533);
        }
    }

    public void addScore(int points)//añade el puntaje al mundo
    {
        score = AsiaW4.score + points; //suma los puntos correctos
